...
interface LichessUserInfo {
  ...
  username: string;
  perfs: object;
  // Add other properties based on the Lichess API response
}
...
async function fetchLichessUserInfo(
  lichessHandle: string,
): Promise<LichessUserInfo> {
  try {
    const headers = {
    ...
    const response = await fetch(
      `https://lichess.org/api/user/${lichessHandle}`,
      { headers },
    );

    if (!response.ok) {
      throw new Error(
        "Failed to fetch Lichess user information: " + response.statusText,
      );
    }

    const userInformation = await response.json();
    return userInformation as LichessUserInfo;
  } catch (error) {
...
async function createChallenge(
  ...
  try {
    ...
    return challengeData as LichessChallengeResponse;
  } catch (error) {
...
export { fetchLichessUserInfo, createChallenge };
