...
const createGame = async (whiteUserId, timeControl, wagerSize) => {
    try {
        ...
        if (checkIfUserHasActiveGame.rows.length > 0) {
            console.log('createGame: User has an active game', checkIfUserHasActiveGame.rows);
...
const getAvailableGames = async (timeControl, wagerSize, userId, userRating) => {
    console.log(`getAvailableGames: timeControl=${timeControl}, wagerSize=${wagerSize}, userId=${userId}, userRating=${userRating}`);
