const db = require('../db');
const { GameNotFoundError, InvalidGameStateError } = require('./errors');
...
async function handleFundsTransferred(to, amount, gameId, wss) {
    ...
    const message = JSON.stringify({ type: 'FUNDS_TRANSFERRED', userID: player.user_id, to, amount: amountString });
...
function determineWinner(gameInfo) {
    return gameInfo.Result === '1-0' ? gameInfo.White : 
           gameInfo.Result === '0-1' ? gameInfo.Black : 
           'Draw';
}
...
function notifyClientsOfGameOver(wss, gameId, winnerHandle) {
    const gameOverMessage = JSON.stringify({ type: 'GAME_OVER', gameId, winner: winnerHandle });
    wss.clients.forEach(client => {
        if (client.readyState === WebSocket.OPEN) {
